...
var $author$project$Home$randomPosition = function (model) {
	return A3(
		...
		A2(
			$elm$random$Random$float,
			$author$project$Box$max(model.g.s) * 0,
			$author$project$Box$max(model.g.s) * 1),
		A2(
			$elm$random$Random$float,
			$author$project$Box$max(model.g.w) * 0,
		...
		switch (msg.$) {
			case 0:
				...
				var particles = (dt < 150) ? A2(
					$elm$core$List$map,
					$author$project$Particle$updateVelocity(scaledDt),
					A2(
						...
						collisionHandler(
							A2(
								...
								A2(
									$elm$core$List$map,
									$author$project$Particle$updatePosition(scaledDt),
									model.z))))) : model.z;
				return _Utils_Tuple2(
					_Utils_update(
						model,
	...
	return _List_fromArray(
		[
			...
			$elm$core$String$fromFloat(
				$author$project$Box$max(model.g.s))),
			A2(
			$elm$html$Html$Attributes$style,
			'height',
			$elm$core$String$fromFloat(
				$author$project$Box$max(model.g.w))),
...
var $author$project$Link$toTexts = F2(
	function (box, links) {
		var textX = $author$project$Box$max(box.s) / 5;
		var range = $author$project$Box$max(box.w);
