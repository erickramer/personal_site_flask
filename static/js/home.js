...
var $author$project$ParticlePage$Model = F3(
	function (box, particles, g) {
		return {aW: box, Y: g, bm: particles};
	});
...
var $elm$browser$Browser$AnimationManager$State = F3(
	function (subs, request, oldTime) {
		return {ab: oldTime, aI: request, aP: subs};
	});
...
var $author$project$Particle$Particle = F4(
	function (position, velocity, acceleration, mass) {
		return {W: acceleration, h: mass, m: position, p: velocity};
	});
...
var $author$project$ParticlePage$randomPosition = function (model) {
	return A3(
		...
		A2(
			$elm$random$Random$float,
			$author$project$Box$max(model.aW.r) * 0,
			$author$project$Box$max(model.aW.r) * 1),
		A2(
			$elm$random$Random$float,
			$author$project$Box$max(model.aW.v) * 0,
			$author$project$Box$max(model.aW.v) * 1));
};
var $author$project$ParticlePage$randomVelocity = function (model) {
	var vy = 2.0e-5 * $author$project$Box$max(model.aW.v);
	var vx = 2.0e-5 * $author$project$Box$max(model.aW.r);
	return A3(
		$elm$random$Random$map2,
		$author$project$Vector$vector,
...
var $author$project$ParticlePage$newParticle = function (model) {
	return A4(
		...
		$author$project$ParticlePage$randomMass(model.aW));
};
		...
		return _Utils_update(
			p,
			{
				p: A2(
					$author$project$Vector$add,
					p.p,
					A2($author$project$Vector$multiply, dt, p.W))
			});
	});
				...
				var particles = (dt < 150) ? A2(
					...
					A2(
						...
						collisionHandler(
							A2(
								$elm$core$List$map,
								$author$project$Box$bounce(model.aW),
								A2(
									$elm$core$List$map,
	...
	return _List_fromArray(
		[
			...
			$elm$core$String$fromFloat(
				$author$project$Box$max(model.aW.r))),
			A2(
			$elm$html$Html$Attributes$style,
			'height',
			$elm$core$String$fromFloat(
				$author$project$Box$max(model.aW.v))),
...
var $author$project$Home$svgElements = function (model) {
	...
	return _Utils_ap(
		A2(
			...
			model.bm),
		A2($author$project$Link$toTexts, model.aW, $author$project$Home$links));
};
...
var $author$project$Home$main = $elm$browser$Browser$element(
	{
		...
		bp: $author$project$ParticlePage$subscriptions($author$project$Home$newWindowSize),
		bt: $author$project$ParticlePage$update,
		bu: $author$project$Home$view
	});
