...
var $author$project$About$ParticlesLoaded = function (a) {
	return {$: 4, a: a};
};
	...
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				...
				$author$project$About$loadParticles($author$project$About$ParticlesLoaded)
			]));
};
...
var $author$project$Box$bounceY = F2(
	function (box, particle) {
		if (_Utils_cmp(
			...
			$author$project$Box$max(box.q)) > 0) {
			var newVelocity = _Utils_Tuple2(
				$author$project$Vector$x(particle.aS),
				-$elm$core$Basics$abs(
					$author$project$Vector$y(particle.aS)));
			var newPosition = _Utils_Tuple2(
				...
				$author$project$Box$max(box.q) - $author$project$Particle$radius(particle));
			return _Utils_update(
				particle,
				{aD: newPosition, aS: newVelocity});
		} else {
			if (_Utils_cmp(
				$author$project$Vector$y(particle.aD) - $author$project$Particle$radius(particle),
				$author$project$Box$min(box.q)) < 0) {
				var newVelocity = _Utils_Tuple2(
					$author$project$Vector$x(particle.aS),
					$elm$core$Basics$abs(
						$author$project$Vector$y(particle.aS)));
				var newPosition = _Utils_Tuple2(
					$author$project$Vector$x(particle.aD),
					$author$project$Box$min(box.q) + $author$project$Particle$radius(particle));
				return _Utils_update(
					particle,
					{aD: newPosition, aS: newVelocity});
			} else {
...
var $author$project$About$randomMass = function (box) {
	return A2(
		...
		A2(
			...
			2,
			$author$project$Box$max(box.p) / 100));
};
...
var $author$project$About$randomPosition = function (model) {
	return A3(
		...
		A2(
			$elm$random$Random$float,
			$author$project$Box$max(model.e.q) * 0,
			$author$project$Box$max(model.e.q) * 1));
};
var $author$project$About$randomVelocity = function (model) {
	var vy = 2.0e-5 * $author$project$Box$max(model.e.q);
	var vx = 2.0e-5 * $author$project$Box$max(model.e.p);
	return A3(
		$elm$random$Random$map2,
		$author$project$Vector$vector,
		...
		switch (msg.$) {
			...
			case 3:
				...
				var newBox = A2(
					...
					model.e,
					_Utils_Tuple2(windowSize.q, windowSize.p));
				return $elm$core$List$isEmpty(model.w) ? _Utils_Tuple2(
					_Utils_update(
						model,
						{e: newBox}),
					A2(
...
var $author$project$About$svgAttributes = function (model) {
	return _List_fromArray(
		[
			...
			'height',
			$elm$core$String$fromFloat(
				$author$project$Box$max(model.e.q))),
